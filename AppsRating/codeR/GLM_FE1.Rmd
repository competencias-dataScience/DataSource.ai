...
- **Tuning:** tiempo de ejecución aproximado de  0.71 horas.
...
```{r}
write_csv(sub_87_glm_fe1, file = "../submission/sub_87_glm_fe1.csv")
```
...
```{r}
write_csv(sub_88_glm_fe1, file = "../submission/sub_88_glm_fe1.csv")
```
...
```{r}
write_csv(sub_89_glm_fe1, file = "../submission/sub_89_glm_fe1.csv")
```
...
```{r}
write_csv(sub_90_glm_fe1, file = "../submission/sub_90_glm_fe1.csv")
```

# Exportando probabilidades

```{r}
write_csv(predichos_final2, file = "../probabilitys/prob_glm_fe1.csv")
```

# Conclusiones

- Las nuevas características (feature engineering) proporcionan mejores resultados respecto al ajuste inicial.
- Con este modelo base se pasa de 0.54414669767945 a 0.584569209.
- La importancia de variables muestra que las tasas obtenidas entre *reviews*, *installs* y *size_app* son fructiferas para el modelamiento.
- El límite (thershold) de probabilidad mejora cuando se disminuye de 0.5 (por defecto) a 0.45 o 0.4 (dependiendo del modelo). Esto es un resultado que sugiere que la calibración de la probabilidad podría ser adecuada.
