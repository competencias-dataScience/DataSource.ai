...
- **Tuning:** tiempo de ejecución aproximado de  1 hora. Incluir el parámetro *`stop_iter = 50`* mejoró los tiempos de entrenamiento, pasando de 8.5 horas a 1 hora.
...
```{r}
# Mejores parámetros
mejor_f1 <- select_best(xgb_tuned, metric = "f_meas")
...
```{r}
write_csv(sub_95_xgb_fe1, file = "../submission/sub_95_xgb_fe1.csv")
```
...
```{r}
write_csv(sub_96_xgb_fe1, file = "../submission/sub_96_xgb_fe1.csv")
```
...
```{r}
write_csv(sub_97_xgb_fe1, file = "../submission/sub_97_xgb_fe1.csv")
```
...
```{r}
write_csv(sub_98_xgb_fe1, file = "../submission/sub_98_xgb_fe1.csv")
```

# Exportando probabilidades

```{r}
write_csv(predichos_final2, file = "../probabilitys/prob_xgb_fe1.csv")
```
